...
export default function LeaveRulesCard({ inProbation, availablePaid, monthsSinceJoining }) {
  const percentageUsed = inProbation ? 0 : Math.min(100, (1 - availablePaid) * 100);
        ...
        <div className="bg-green-50 border border-green-100 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-green-800 text-[15px] font-semibold flex items-center gap-1.5">
              <CheckCircle className="w-4 h-4" /> Monthly Paid Leave Quota
            </p>
          ...
          <p className="text-green-700 text-sm mt-3 flex justify-between">
            <span>Quota: 1 paid leave/month</span>
            <span className="text-xs opacity-70 font-medium">{1 - availablePaid} used</span>
          </p>
        </div>
      )}
    </div>
